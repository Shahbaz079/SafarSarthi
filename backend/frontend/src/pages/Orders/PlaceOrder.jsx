...
import { clearbookedItems, removeFromBookings } from "../../redux/api/features/cart/cartSlice";
import { AnimatedModalDemo } from "../../Components/ui/animated-modal/Animated-modelDemo";
const PlaceOrder = () => {
  ...
  const shippingAddress=bookings.shippingAddress

  
  

  useEffect(() => {
    if (!bookings.shippingAddress.address) {
      navigate("/booking");
    }
  }, [bookings.paymentMethod, bookings.shippingAddress.address, navigate]);
  ...
  const placeOrderHandler = async () => {
    try {
      const res = await createOrder({
        orderItems: bookings.bookedItems,
        shippingAddress: bookings.shippingAddress,
        paymentMethod: bookings.paymentMethod,
        itemsPrice: bookings.itemsPrice,
        
        taxPrice: bookings.taxPrice,
        totalPrice: bookings.totalPrice,
      }).unwrap();
      dispatch(clearbookedItems());
...
dispatch(removeFromBookings(id));
}
  return (
    <>
      

<div className="container lg:ml-[6rem] mx-[1.5rem] mt-8 w-[80vw]">
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr>
            ...
            <td className="px-1 py-2 text-left">Remove Package</td>
          </tr>
        </thead>

        <tbody>
          {bookings.bookedItems?.map((item, index) => (
            <tr key={index}>
              <td className="p-2">
                <img
                  src={item.image}
              ...
              <td className="p-2" onClick={()=>deleteHandler(item._id)}><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 100 100">
    ...
    <div className="flex justify-between flex-wrap p-8 bg-[#181818]">
      ...
      <div>
        ...
        <p>
          ...
          {shippingAddress.country} <br /> Contact No:  {shippingAddress.contact}
        </p>
      </div>
{/*
      <div>
        <h2 className="text-2xl font-semibold mb-4">Payment Method</h2>
        <strong>Method:</strong> {bookings.paymentMethod}
      </div>*/}
    </div>
{/*
    <button
      ...
      disabled={bookings.bookedItems === 0}
      onClick={placeOrderHandler}
    >
      Place Order
    </button>
*/}
   <AnimatedModalDemo bookings={bookings} user={userInfo} />

...
const handleCreateCategory=async(e)=>{
  ...
  if(!name){
    toast.error('Loaction Name Required')
    return;
  }
  ...
  try {
    ...
    console.error(error)
    toast.error("Location Creation Failed,try again")
  }
}
        ...
        <div className="flex flex-wrap">
          {categories?.map((category)=>(
            <div key={category._id}>
              <button className="bg-white border border-pink-500 text-pink-500 
              py-2 px-4 rounded-lg m-3 hover:bg-pink-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50" onClick={()=>{
                {
